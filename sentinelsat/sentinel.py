...
class SentinelAPI:
    ...
    check_certificate : bool
        Whether to check the hosts ssl certificate or not for all session requests.
        default to False.
    ...
    def __init__(self, user, password, api_url='https://scihub.copernicus.eu/apihub/',
                 show_progressbars=True, check_certificate=True):
    ...
    def _download(self, url, path, session, file_size):
        ...
        downloaded_bytes = 0
        with closing(session.get(url, stream=True, auth=session.auth, headers=headers, verify=self.check_certificate)) as r, \
